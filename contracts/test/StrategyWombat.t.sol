...
contract StrategyWombatTest is Test {
    ...
    function testStrategyWombatBasicFunctions() public {
        // اختبار دوال StrategyWombat الأساسية
        assertEq(
            strategy.vault(),
            address(vault),
    ...
    function testWombatAMMRates() public {
        ...
        // Wombat yields should be competitive (11% APY)
        assertGt(yield, 0, "Wombat should generate some yield");
    }

    function testWombatStability() public {
        // اختبار استقرار Wombat
        vm.deal(user1, 10 ether);
        vm.prank(user1);
    ...
    function testWombatLiquidityProvision() public {
        // اختبار توفير السيولة في Wombat
        ...
        // Wombat should benefit from increased trading activity
        assertGt(yieldAfter, 0, "Wombat should maintain positive yield");
    }

    function testWombatImpermanentLoss() public {
        // اختبار الخسارة المؤقتة في Wombat
        vm.deal(user1, 10 ether);
        vm.prank(user1);
        vault.deposit{value: 5 ether}();
        ...
        // Wombat should handle impermanent loss gracefully
        assertGt(
            yieldAfterVolatility,
            0,
            "Wombat should maintain positive yield despite volatility"
        );
    }
}
